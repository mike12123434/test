...
def generate_cluster_descriptions(
    ...
    Calculates Mean for numeric and Mode for categorical.
    """
    desc_text = {}
...
def generate_anomaly_insights(
    ...
    """
    Generate AI-powered insights about detected anomalies
    """
...
def main():
    ...
    with st.sidebar:
        ...
        gemini_api_key = st.text_input("Google Gemini API Key (選填)", type="password")
        if gemini_api_key:
             st.caption("已輸入 API Key，將啟用 AI 洞察功能")
    ...
    if st.session_state.get('ran_clustering'):
        ...
        if best_res and best_res['score'] > 0:
            ...
            # Generate descriptions AND the table
            descriptions, cluster_stats_df = generate_cluster_descriptions(
                df_viz, selected_features, overall_means, gemini_api_key
            )
            ...
            if gemini_api_key:
                ...
                st.info("💡 提供 Gemini API Key 以啟用 AI 智能分析功能")
                st.markdown("""
                **AI 分析功能包括：**
                - 🎯 自動識別各群組的核心特徵
                - 📊 比較不同群組的差異
            ...
            if gemini_api_key:
                ...
                with st.spinner("正在分析異常資料..."):
                    insights = generate_anomaly_insights(
                        anomalies, selected_features, gemini_api_key
                    )
                    st.info(insights)
